...
function convert(mcharacter) {
    ...
    for (var i=0; i < characterlength; i++) {
      ...
      const check = characterresult.length;
         if(check <= 0){
         console.error(`(Password-Locking): The system could not read text`);
          return "0"
        }
        if (check > 0) {
    ...
    function translation(mcharacter) {
      ...
      async function readfile_convert(fileurl , value) {
        fs.readFile(`./${fileurl}`,'utf8', (err, fltext)=>{ 
          if(err){
            if(err.errno == -4058){
              console.error("(Password-Locking): the system could not find the file path")
            }
          }else{

        const result = convert(fltext);
        if(value == false){
        return result;
        }
        if(value == true){  
      ...
      async function readfile_translation(fileurl , value) {
       fs.readFile(`./${fileurl}`,'utf8', (err, fltext)=>{
        if (err) {
          if (err.errno == -4058) {
            console.error("(Password-Locking): the system could not find the file path")
          ...
          const data = fltext.toString();
          const result = translation(data);
          if (result == "0") {
            console.error("(Password-Locking): the system could not read the file text")
          }else{
            if (value == false) {
              return result
            }
